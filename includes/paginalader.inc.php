...
function inhoudCall(PDO $dbh, $page) {
    if ($page == "Over mij") {
        ...
        $sth2 = $dbh->prepare("SELECT a.afbeelding FROM afbeelding a LEFT JOIN pagina p ON a.afbeeldingid = p.afbeelding WHERE titel = ?");
        ...
        $sth->execute(array($page));
        while ($result = $sth->fetch(PDO::FETCH_ASSOC)) {
            echo ("<div class='behandeling'><div class='behandeling-text'><h1>" . $result['titel'] . "</h1>");
            if ($result['afbeelding'] != NULL) {
                print('<img src="' . substr($result['afbeelding'], 3) . '" alt="Nieuws item">');
            } echo("<p>" . $result['inhoud'] . "</p><a href='behandeling-overzicht.php?behandeling=" . $result['behandeling_id'] . "'>Lees meer</a></div></div>");
        }
        ...
        while ($rows = $stmt->fetch()) {
            print('<div class="left"><h1>' . $rows['titel'] . '</h1><p>' . $rows['inhoud'] . '</p></div><div class="right"><h1>Afbeelding</h1><section class="gallery"><div><img src="');
            echo $rows['afbeelding'];
            print('" alt="Nieuws bericht"></div></section>');
        }
    } elseif ($page == "Nieuws") {
        $sth = $dbh->prepare("SELECT titel, inhoud FROM pagina WHERE titel = ?");
        $sth->execute(array($page));
        ...
        $stmt->execute();
        while ($rows = $stmt->fetch()) {
            print('<div class="behandeling"><div class="behandeling-text"><h1>' . $rows['titel'] . '</h1> ');
            if ($rows['afbeelding'] != NULL) {
                print('<img src="' . substr($rows['afbeelding'], 3) . '" alt="Nieuws item">');
            } print(' <p>' . $rows['inhoud'] . '</p><p class="datum">' . $rows['datum'] . '</p><a href="nieuws-overzicht.php?nieuwsitem=' . $rows['nieuws_id'] . '">Lees meer ></a></div></div>');
        }
...
function contactgegeven($dbh) { //functie om contactgegevens uit de db te halen, krijgt de database connectie mee in $DBH
    $sth = $dbh->prepare("SELECT email, telnummer, adres FROM contactgegeven"); 
    $sth->execute();

    $result = $sth->fetch(PDO::FETCH_ASSOC);
    return($result); //stuur de array met de gegevens terug
}
